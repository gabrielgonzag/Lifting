create or replace function public.lifting_iron_streak_multiplier(p_weekly_workouts integer)
returns numeric
language sql
immutable
as $$
  select case
    when p_weekly_workouts >= 6 then 1.8
    when p_weekly_workouts >= 5 then 1.5
    when p_weekly_workouts >= 4 then 1.25
    when p_weekly_workouts >= 3 then 1.1
    else 1.0
  end
$$;
...
begin
  ...
  if v_streak >= 3 then v_total_xp := v_total_xp + 60; end if;
  if v_streak >= 7 then v_total_xp := v_total_xp + 165; end if;
  if v_streak >= 14 then v_total_xp := v_total_xp + 365; end if;
  if v_streak >= 30 then v_total_xp := v_total_xp + 1020; end if;
  ...
  v_total_xp := round(v_total_xp * public.lifting_iron_streak_multiplier(v_weekly_workouts));
  ...
  from (values
    ...
    ('seven-day-streak', 165, v_streak >= 7),
    ('thirty-day-streak', 1020, v_streak >= 30),
    ('ten-prs', 0, v_prs >= 10),
    ('fifty-prs', 0, v_prs >= 50),
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'total_xp', v_total_xp,
      'title', v_title,
      'iron_streak_multiplier', public.lifting_iron_streak_multiplier(v_weekly_workouts)
    )
  );
